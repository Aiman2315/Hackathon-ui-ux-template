...
import { getCardItems, removeFromCart, updateCartQuantity } from '@/app/addtocart/addtocart'
...
const page = () => {
  ...
  const handleQuantityChange = (productId: string, newQuantity: number) => {
    ...
    const updatedItems = getCardItems();
        ...
        <div className='flex contain-layout sm:flex-row flex-col w-full h-[525px] justify-between  items-center gap-3 p-44 mb-32'>
          <div className='flex flex-row sm:flex-row justify-between items-center'>
            <div className='sm:w-[817px] w-[350px] h-[216px] translate-y-[-100px] flex flex-col '>
              ...
              <div className='w-full h-[55px] bg-main p-4 flex flex-row sm:gap-16 gap-10'>
                <Image src={item.image} height={60} width={150} alt='asgaard' className='bg-[#FBEBB5] rounded-md sm:h-[106px] h-[50px] sm:w-[106px] w-[50px]'/>
                <div className='flex flex-row justify-start pt-16 items-end sm:gap-20 gap-10'>
                  <h1 className='text-[#9F9F9F] sm:text-base text-sm'>{item.name}</h1>
                  <h1 className='text-[#9F9F9F] sm:text-base text-sm'>{item.price}</h1>
                  <div className='border-2 sm:h-[32px] h-[26px] sm:w-[32px] w-[26px] sm:text-base text-center rounded-lg text-black px-2 translate-y-1'>{item.inventory}</div>
                  <h1 className='text-black sm:text-base text-sm font-normal'>{`${
                      totalPrice = cartItems.reduce((sum, item) => sum + item.price * item.inventory, 0)               
                  }`}</h1>
                  <Image src="/Delete.png" height={20} width={20} alt='delete' className='sm:mt-1 mt-0 pt-5' onClick={() => handleRemoveItem(item._id)}></Image>
                </div>
          ...
          <div className='sm:h-[390px] h-[359px] sm:w-[393px] w-[300px] bg-[#FFF9E5] rounded-lg flex flex-col mb-6 sm:gap-8 gap-2 justify-center items-center py-7'>
            ...
            <div className='flex flex-row justify-center items-start w-full p-3 gap-16'>
              ...
              <h1 className='font-medium sm:text-lg text-sm text-[#9F9F9F]'>{totalPrice}</h1>
            </div>
            <div className='flex flex-row justify-center items-start w-full p-3 gap-16 '>
              <h1 className='font-medium  text-lg'>Total</h1>
              <h1 className='font-medium text-2xl text-[#B88E2F]'>{totalPrice}</h1>
            </div>
